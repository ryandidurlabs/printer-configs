...
# Verify Printer-2 directory exists
if [ ! -d "$CONFIG_DIR/Printer-2" ]; then
    echo -e "${RED}Error: Printer-2 directory not found in repository${NC}"
    exit 1
fi

# Copy Printer-2 configuration files
echo -e "${BLUE}Copying Printer-2 configuration files...${NC}"
cp "$CONFIG_DIR/Printer-2/printer.cfg" "$KLIPPER_CONFIG_DIR/printer.cfg"
if [ -f "$CONFIG_DIR/Printer-2/macros.cfg" ]; then
...
echo "Files copied:"
echo "  - printer.cfg"
[ -f "$CONFIG_DIR/Printer-2/macros.cfg" ] && echo "  - macros.cfg"
